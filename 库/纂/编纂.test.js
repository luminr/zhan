...
const 定位 = (谓) => {
	...
	const 媒体类型 = 标签映射至媒体类型[
		/** @type {keyof typeof 标签映射至媒体类型} */ (元素?.tagName.toLowerCase())
	];
...
Deno.test("模块：嵌套：不迭代", 生成测试("模块：嵌套：不迭代", { 替换迭代: false }));
