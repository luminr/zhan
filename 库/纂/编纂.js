...
) => ({ 关键字, 前端报错, 替换迭代上限, 定位, 前端报错模板, 脚本工具箱, 包括头部 });
...
export async function 编纂超文本(模块, 用户配置) {
	const 运行时缓存 = new Map();
	const 配置 = 抄写配置(用户配置);
	const 文档范围 = 配置.包括头部 ? (new DOMParser()).parseFromString(模块.内容, "text/html") : 做片段(模块);
	await 元素编纂(文档范围, 配置, 运行时缓存);
	文档范围.querySelectorAll(全选择器(配置.关键字)).forEach((元素) => 元素.remove());
	return (配置.包括头部 ? DOCTYPE : "") + 做超文本([...文档范围.childNodes]);
}
	...
	for (const 元素 of 文档范围.querySelectorAll(全选择器(配置.关键字))) {
		try {
			...
			if ((模块标签 === "script") && (模块类型 === "module")) {
				...
				await 脚本模块({ document: 元素.getRootNode(), Zhan: 配置.脚本工具箱 });
			} else if (模块地址) {
				...
				} else if (插入元素.nodeName === "#document-fragment") {
					元素.replaceWith(插入元素);
				} else {
